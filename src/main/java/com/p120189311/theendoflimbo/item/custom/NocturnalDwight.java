...
public class NocturnalDwight extends Item {
                ...
                for (int dy = -1; dy <= 1; dy++) {
                    for (int dz = -1; dz <= 1; dz++) {
                        ...
                        if (state.is(Blocks.DIAMOND_ORE) || state.is(Blocks.DEEPSLATE_DIAMOND_ORE)) {
                            level.setBlock(targetPos, TheEndOfLimboBlocks.ABSOLUTE_BLACK_ORE.get().defaultBlockState(), 3);
                            blockTransformed = true;
                        } else if (state.is(Blocks.EMERALD_ORE) || state.is(Blocks.DEEPSLATE_EMERALD_ORE)) {
                            level.setBlock(targetPos, TheEndOfLimboBlocks.ABSOLUTE_WHITE_ORE.get().defaultBlockState(), 3);
                            blockTransformed = true;
                        } else if (state.is(TheEndOfLimboBlocks.ABYSS_PLANKS_CURSED.get())) {
                            level.setBlock(targetPos, TheEndOfLimboBlocks.ABYSS_PLANKS.get().defaultBlockState(), 3);
                            blockTransformed = true;
                        }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.translatable("tooltip.theendoflimbo.nocturnal_dwight").withStyle(ChatFormatting.LIGHT_PURPLE).withStyle(ChatFormatting.ITALIC).withStyle(ChatFormatting.BOLD));
    }
}
