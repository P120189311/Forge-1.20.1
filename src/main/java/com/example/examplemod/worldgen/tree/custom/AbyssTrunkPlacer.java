...
public class AbyssTrunkPlacer extends TrunkPlacer {
    ...
    protected TrunkPlacerType<?> type() {
        return ModTrunkPlacerTypes.ABYSS_TRUNK_PLACER.get();
    }

    /* ---------------------- Small helpers ---------------------- */
    private boolean isAirOrSmallObjects(LevelSimulatedReader reader, BlockPos pos) {
        return reader.isStateAtPosition(pos, state ->
                state.isAir() || state.canBeReplaced() || state.is(ModTags.Blocks.REPLACEABLE_PLANTLIKE));
    }
                ...
                for (int dz = -radius; dz <= radius; dz++) {
                    ...
                    if (layer < 6) {
                        ...
                        if (reader.isStateAtPosition(under, s ->
                                s.isAir() || s.canBeReplaced() || s.is(ModTags.Blocks.REPLACEABLE_PLANTLIKE))) {
